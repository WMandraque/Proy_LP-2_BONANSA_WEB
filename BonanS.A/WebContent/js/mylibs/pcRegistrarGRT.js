		...
		$.ajax({
			...
			success : function(r) 
			{
									...
									for (var i = 0; i < json.length; i++) 
									{
										...
										cadena = cadena + "<td><a class='eliminarGRyFACT' onclick='fn_eliminarGRyFACT()'><input type='button' class='btn btn-sm' value='ELIMINAR'/></a></td>";
...
function fn_eliminarGRyFACT() {
	$("a.eliminarGRyFACT").click(function(){
		descTraslado = $(this).parents("tr").find("td").eq(0).html();
		txtNumGr = document.getElementById("txtNumGr").value;
		txtNumFact = document.getElementById("txtNumFact").value;
		$.ajax({
			url : 'gestionaGRT?operacion=eliminarGRyFACT',
			type : 'post',
			datatype : 'json',
			data :   "descTraslado="+descTraslado
			        +"&txtNumGr="+txtNumGr
